      ...
      <div className='p-4 h-screen max-w-full lg:max-w-[40rem] bg-white flex flex-col justify-between overflow-y-scroll'>
        <div className='flex flex-col gap-[5rem]'>
          <DominantColor imageUrl={pokemonData.image}>
            <div onClick={handleClose} className='h-[3rem] w-[3rem] bg-white shadow-xl rounded-lg flex items-center justify-center cursor-pointer'>
              <BackspaceIcon className="text-black h-3 w-3"/>
            </div>
            <div className='flex justify-center'> 
              <Image src={pokemonData.image} alt={pokemonData.name} width={40} height={40} className='relative h-[20rem] w-[20rem] top-10 ' /> 
            </div>
          </DominantColor>
